...
pub fn setup_and_run(query) {
  ...
  let assert Ok(_) =
    test_data.insert_owners_rows() |> sqlite.execute_raw_sql(conn)

  let assert Ok(_) =
    test_data.drop_cats_table_if_exists() |> sqlite.execute_raw_sql(conn)
  let assert Ok(_) =
    test_data.create_cats_table() |> sqlite.execute_raw_sql(conn)
  let assert Ok(_) =
    test_data.insert_cats_rows() |> sqlite.execute_raw_sql(conn)

  let assert Ok(_) =
    test_data.drop_dogs_table_if_exists() |> sqlite.execute_raw_sql(conn)
  let assert Ok(_) =
    test_data.create_dogs_table() |> sqlite.execute_raw_sql(conn)
  let assert Ok(_) =
...
pub fn setup_and_run_write(query) {
  ...
  let assert Ok(_) =
    test_data.insert_counters_rows() |> sqlite.execute_raw_sql(conn)
